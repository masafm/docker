...
for task in clean build run;do
    ...
    docker run --rm -u gradle -v "${PWD}/src/jmx-test":/home/gradle/project -v "${PWD}/dd-java-agent.jar:/opt/dd-java-agent.jar" -w /home/gradle/project gradle:8.5.0-jdk17 gradle $task
    if [[ $? != 0 ]];then
